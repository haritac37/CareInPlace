...
// ─── LANDING ──────────────────────────────────────────────────────────────────
function Landing({onSync,onGetStarted}) {
    ...
    <div style={{minHeight:"100%",background:T.bg,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        <div style={{padding:"8px 22px 32px",opacity:vis?1:0,transform:vis?"none":"translateY(14px)",transition:"all .6s ease"}}>
          ...
          {[
            {icon:"▶", label:"Start & complete daily sessions",    sub:"Step-by-step exercise player with timer"},
            {icon:"☀", label:"Daily energy check-in",             sub:"Tells us how you feel — we adjust accordingly"},
            {icon:"📈", label:"Track your streak & mobility score", sub:"See your consistency build week by week"},
            {icon:"🔔", label:"WhatsApp reminders",               sub:"Gentle nudges on your session days"},
          ].map((f,i)=>(
            <div key={i} style={{display:"flex",gap:14,alignItems:"flex-start",marginBottom:16,paddingBottom:16,borderBottom:i<3?`1px solid ${T.border}`:"none"}}>
              <div style={{width:36,height:36,borderRadius:10,background:T.surfaceAlt,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:15}}>{f.icon}</div>
              <div>
                <p style={{fontSize:13,fontWeight:500,color:T.espresso,marginBottom:2}}>{f.label}</p>
...
function CheckIn({prog,onDone}) {
  ...
  return(
    <div style={{minHeight:"100%",background:T.bg,display:"flex",flexDirection:"column"}}>
      <SBar/>
      <div style={{flex:1,padding:"12px 22px 24px",display:"flex",flexDirection:"column"}}>
        ...
        <p style={{fontSize:14,color:T.muted,marginBottom:28}}>How are you feeling before today's session?</p>

        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:28}}>
          {opts.map(o=>{
            const s=energy===o.v;
            return <button key={o.v} onClick={()=>setEnergy(o.v)} style={{padding:"16px 18px",textAlign:"left",background:s?`${o.color}12`:T.surface,border:`1.5px solid ${s?o.color:T.border}`,borderRadius:14,cursor:"pointer",display:"flex",alignItems:"center",gap:14,transition:"all .15s",boxShadow:s?T.shadow:"none"}}>
              <div style={{width:40,height:40,borderRadius:"50%",background:s?o.color:T.surfaceAlt,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"background .15s"}}>
                <div style={{width:14,height:14,borderRadius:"50%",background:s?"white":o.color,opacity:s?1:.6}}/>
              </div>
              <div>
                <p style={{fontSize:14,fontWeight:500,color:s?o.color:T.espresso,marginBottom:2}}>{o.label}</p>
        ...
        {/* Session preview */}
        {prog?.sessions?.[0]&&(
          <div style={{background:T.surfaceAlt,borderRadius:14,padding:"14px 16px",marginBottom:20,border:`1px solid ${T.border}`}}>
            <p style={{fontSize:11,color:T.muted,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>Today's session</p>
        ...
        <PBtn label={energy?"Start Today's Session →":"Select how you're feeling first"} onClick={()=>energy&&onDone(energy)} disabled={!energy}/>
        <button onClick={()=>onDone("skip")} style={{background:"none",border:"none",cursor:"pointer",color:T.mutedL,fontSize:12,marginTop:12,textAlign:"center",width:"100%"}}>Skip check-in</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const handleSync=(email)=>{
    ...
    setScreen("loading");
    setTimeout(()=>{
      // Simulate synced account — in production this calls your API
  ...
  return(
    <Shell>
      {screen==="landing"   && <Landing onSync={()=>setScreen("sync")} onGetStarted={()=>setScreen("onboarding")}/>}
      {screen==="sync"      && <SyncScreen onDone={handleSync}/>}
      {screen==="onboarding"&& <Onboarding onDone={handleOnboarding}/>}
      {screen==="loading"   && <Loading message={loadMsg}/>}
      {screen==="checkin"   && prog && <CheckIn prog={prog} onDone={(e)=>{ if(e!=="skip"&&prog){const adj={...prog,sessions:[{...prog.sessions[0],intensity:e==="low"?"Gentle":e==="good"?"Moderate":prog.sessions[0].intensity},...prog.sessions.slice(1)]};setProg(adj);} setScreen("player"); }}/>}
